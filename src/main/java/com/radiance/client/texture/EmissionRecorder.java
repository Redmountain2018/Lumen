...
public final class EmissionRecorder {
    ...
    private static List<EmissionCell> buildRegionCells(NativeImage albedo,
        ...
        // Use direct memory access to avoid JNI per-pixel overhead from getColorArgb().
        // NativeImage stores pixels as ABGR (4 bytes/pixel) in native byte order.
        // Cast via Object first because INativeImageExt is mixed into NativeImage at runtime.
        long albedoPtr = ((INativeImageExt) (Object) albedo).neoVoxelRT$getPointer();
        long specularPtr = ((INativeImageExt) (Object) specular).neoVoxelRT$getPointer();
        int albedoWidth = albedo.getWidth();
        int specularWidth = specular.getWidth();
        final long BYTES_PER_PIXEL = 4L;
            ...
            for (int gridX = 0, tileX = 0; tileX < width; gridX++, tileX += tileStep) {
                ...
                for (int y = 0; y < tileHeight; y++) {
                    for (int x = 0; x < tileWidth; x++) {
                        ...
                        // Read ABGR pixel directly from NativeImage memory (avoids JNI call)
                        long albedoOffset = (sampleY * (long) albedoWidth + sampleX) * BYTES_PER_PIXEL;
                        int abgr = MemoryUtil.memGetInt(albedoPtr + albedoOffset);
                        // Convert ABGR (native storage) to ARGB (expected by channel logic)
                        int argb = (abgr & 0xFF00FF00) | ((abgr & 0xFF) << 16)
                            | ((abgr >>> 16) & 0xFF);

                        float alphaCoverage = ((argb >>> 24) & 0xFF) / 255.0f;
                        if (alphaCoverage <= 0.0f) {
                            continue;
                        }

                        // Read specular alpha byte directly — only alpha needed for emission
                        long specularOffset = (sampleY * (long) specularWidth + sampleX)
                            * BYTES_PER_PIXEL + 3;
                        int specAlpha = MemoryUtil.memGetByte(specularPtr + specularOffset) & 0xFF;
                        float emission = (specAlpha == 255) ? 0.0f : specAlpha / 254.0f;
                        if (emission <= 0.0f) {
                            continue;
                        }
